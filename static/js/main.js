/* OAuth Lab UI — toggle mode, jalankan exploit, render hasil */
(function () {
  ...
  function renderOut(sc, data) {
    ...
    var lines = data.steps.map(function (l) {
      var x = esc(l);
      if (x.indexOf('FLAG') > -1 || x.toLowerCase().indexOf('flag') > -1) x = x.replace(/OAUTH-LAB\{[^}]+\}/g, '<span class="flag">$&</span>');
      if (x.indexOf('RESULT') > -1) x = '<span class="ok">' + x + '</span>';
      if (x.indexOf('BLOCKED') > -1 || x.indexOf('ditolak') > -1 || x.indexOf('DITOLAK') > -1) x = '<span class="bad">' + x + '</span>';
      return '▸ ' + x;
    }).join('\n');
    pre.innerHTML = lines + '\n' + (data.flag ? '\n<span class="flag">🏁 FLAG: ' + esc(data.flag) + '</span>' : '');
  }

  function qs(fetch) { return fetch; }
    ...
    if (tgl) {
      ...
      fetch('/api/toggle/' + sc, { method: 'POST' })
        ...
        .then(function (d) {
          ...
          document.getElementById('out-' + sc).innerHTML = '<span class="dim">▶ mode diganti — jalankan ulang exploit.</span>';
        });
      return;
    }
